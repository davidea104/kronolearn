...
- [ ] T013 [P] [US1] Implementar prueba ejecutable de health check en `tests/smoke/test_health_check.py` que realice una solicitud GET a `/healthz` y verifique: HTTP 200 y cuerpo JSON `{"status": "ok"}`. Integrar esta prueba en el workflow CI definido en `.github/workflows/ci.yml`.
